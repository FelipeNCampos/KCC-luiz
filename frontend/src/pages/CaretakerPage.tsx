...
export function CaretakerPage() {
  ...
  return (
    <DashboardShell title="Contractor" subtitle="Contractor hours, goals and records">
      ...
      {isMaintenanceOpen ? (
        <div className="fixed inset-0 z-40 grid place-items-center bg-black/50 p-4">
          <article aria-modal="true" className="flex max-h-[90dvh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-oak-border bg-white shadow-oakLg" role="dialog" aria-labelledby="maintenance-title">
            <header className="flex items-start justify-between gap-4 px-6 pb-5 pt-6">
              <div>
                <h2 className="text-xl font-extrabold text-oak-coffee" id="maintenance-title">Maintenance</h2>
                <p className="text-sm font-medium text-black/60">Track scheduled contractor maintenance and completed IN/OUT records.</p>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <button className="oak-button-primary !min-h-9 !px-4" type="button" onClick={openMaintenanceAdd}><Plus size={16} />Add</button>
                <button aria-label="Close maintenance" className="grid size-9 place-items-center rounded-lg text-oak-taupe hover:bg-oak-panel" type="button" onClick={() => setIsMaintenanceOpen(false)}><X size={17} /></button>
              </div>
            </header>
            <div className="px-6 pb-4">
              <div className="inline-flex rounded-xl bg-oak-panel p-1">
                <button aria-pressed={maintenanceTab === "schedule"} className={`rounded-lg px-3 py-2 text-sm font-extrabold ${maintenanceTab === "schedule" ? "bg-oak-taupe text-white shadow-sm" : "text-oak-muted hover:text-oak-coffee"}`} type="button" onClick={() => setMaintenanceTab("schedule")}>Schedule</button>
                <button aria-pressed={maintenanceTab === "history"} className={`rounded-lg px-3 py-2 text-sm font-extrabold ${maintenanceTab === "history" ? "bg-oak-taupe text-white shadow-sm" : "text-oak-muted hover:text-oak-coffee"}`} type="button" onClick={() => setMaintenanceTab("history")}>History</button>
              </div>
            </div>
            <div className="min-h-0 overflow-auto px-6 pb-6">
              <div className="overflow-hidden rounded-xl border border-oak-border">
                {maintenanceTab === "schedule" ? <table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-oak-panel text-xs font-extrabold uppercase tracking-wide text-oak-coffee"><tr><th className="px-4 py-3">Category</th><th className="px-4 py-3">Tag</th><th className="px-4 py-3">Report</th><th className="px-4 py-3">Frequency</th><th className="px-4 py-3">Notes</th></tr></thead><tbody>{maintenanceSchedules.length === 0 ? <tr><td className="px-4 py-5 text-black/60" colSpan={5}>No maintenance scheduled.</td></tr> : maintenanceSchedules.map((item) => <tr className={item.is_overdue ? "border-t border-red-200 bg-red-50" : "border-t border-emerald-200 bg-emerald-50"} key={item.id}><td className="px-4 py-3 font-bold">{item.category_name}</td><td className="px-4 py-3">{item.tag}</td><td className="px-4 py-3">{item.report}</td><td className="px-4 py-3">{item.frequency_days} days</td><td className="px-4 py-3">{item.notes}</td></tr>)}</tbody></table> : <table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-oak-panel text-xs font-extrabold uppercase tracking-wide text-oak-coffee"><tr><th className="px-4 py-3">Category</th><th className="px-4 py-3">Tag</th><th className="px-4 py-3">Contractor</th><th className="px-4 py-3">IN</th><th className="px-4 py-3">OUT</th></tr></thead><tbody>{maintenanceHistory.length === 0 ? <tr><td className="px-4 py-5 text-black/60" colSpan={5}>No maintenance history yet.</td></tr> : maintenanceHistory.map((item) => <tr className="border-t border-oak-border" key={item.id}><td className="px-4 py-3">{item.category_name}</td><td className="px-4 py-3">{item.tag}</td><td className="px-4 py-3">{item.contractor_name}</td><td className="px-4 py-3">{formatDate(item.in_at)} {formatTime(item.in_at)}</td><td className="px-4 py-3">{item.out_at ? `${formatDate(item.out_at)} ${formatTime(item.out_at)}` : "Open"}</td></tr>)}</tbody></table>}
              </div>
              {maintenanceTab === "schedule" ? <p className="pt-3 text-xs font-medium text-black/60">Green is on schedule. Red means the last completed maintenance is past its frequency.</p> : null}
            </div>
          </article>
        </div>
      ) : null}
